...
module Puppet::Parser::Functions
  newfunction(:os_patching_pick, :type => :rvalue, :doc => <<-EOS
    @summary
    ...
    ```$real_jenkins_version = os_patching_pick($::jenkins_version, '1.449')```
    > *Note:*
  ...
  EOS
    ...
    args.delete('')
    raise Puppet::ParseError, 'os_patching_pick(): must receive at least one non empty value' if args[0].to_s.empty?
    return args[0]
  end
end
